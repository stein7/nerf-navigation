...
class Planner:
    def __init__(self, start_state, end_state, cfg, density_fn, opt, model):
    ...
    def dh_get_cost_bp(self):
        # density = self.nerf(self.states[:, :3])
        #pdb.set_trace()
        density = self.nerf_bp( self.dh_body_to_world(self.robot_body) )**2
        # print(density.shape)
        colision_prob = torch.mean(density, dim = -1) 
        return colision_prob # colision_prob*1e6
    ...
    def a_star_init(self):
        ...
        if self.opt.a_star_approx: output = self.nerf_astar(coods)
        else: output = self.nerf(coods)
        ...
        if self.opt.bp_approx: self.save_poses(self.basefolder / "init_poses" / (str(f"A_star_approx_path")+".json"))
        else: self.save_poses(self.basefolder / "init_poses" / (str(f"A_star_path")+".json"))
        #nerf_loss = self.dh_get_cost()
    ...
    def dh_learn_init(self):
        ...
        try:
            ...
            # 임계값 설정
            threshold = 0.3
            if self.opt.bp_approx: nerf_loss = self.dh_get_cost_bp()
            else : nerf_loss = self.dh_get_cost()
            ...
            if hasattr(self, "basefolder"):
                if self.opt.bp_approx: 
                    self.save_poses(self.basefolder / "init_poses" / (str("bp_start")+".json"))
                    self.save_costs(self.basefolder / "init_costs" / (str("bp_start")+".json"))
            ...
            while not condition_met:
                ...
                if condition_met == True:
                    ...
                    if hasattr(self, "basefolder"):
                        if self.opt.bp_approx: 
                            self.save_poses(self.basefolder / "init_poses" / (str("bp_end")+".json"))
                            self.save_costs(self.basefolder / "init_costs" / (str("bp_end")+".json"))
    ...
    def nerf_astar(self, input):
        print(f'nerf_astar')
        rot = torch.tensor([[0., 0., 1.], [1., 0., 0.], [0., 1., 0.]], device=device, dtype=torch.float32)
        output = self.model.a_star_density(input.reshape((-1, 3)) @ rot)['sigma'].reshape(input.shape[:-1])
        return output
    def nerf_bp(self, input):
        print(f'nerf_bp')
        rot = torch.tensor([[0., 0., 1.], [1., 0., 0.], [0., 1., 0.]], device=device, dtype=torch.float32)
        output = self.model.bp_density(input.reshape((-1, 3)) @ rot)['sigma'].reshape(input.shape[:-1])
        return output

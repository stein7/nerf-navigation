...
def simulate(planner_cfg, agent_cfg, filter_cfg, extra_cfg, density_fn, render_fn, get_rays_fn):
    ...
    basefolder =  pathlib.Path(planner_cfg['exp_name']) / "paths" 
    ...
    pdb.set_trace(header=f'=========== init Path Planning ===========')
    if opt.a_star_sampling:
        traj.a_star_init()
    elif opt.interp_sampling:
        start_pos = planner_cfg['start_pos']
        end_pos = planner_cfg['end_pos']
        points = planner_cfg['points']
        traj.interp_init(start_pos, end_pos, points)
    elif opt.hybrid_sampling:
        traj.hybrid_init()
        
    elif opt.interp_dense:
        start_pos = planner_cfg['start_pos']
        end_pos = planner_cfg['end_pos']
        points = planner_cfg['points']
        traj.interp_dense_init(start_pos, end_pos, points)
        
    elif opt.random_path:
        traj.dh_a_star_init_random()
    elif opt.a_star_sdf:
        traj.a_star_sdf()
    else:
        return
    ...
    pdb.set_trace(header=f'=========== Path Tunning ===========')
    if opt.tuning_default:
        traj.learn_init()
    elif opt.num_grad_tuning:
        traj.dh_learn_init2()
    else:
        traj.dh_learn_init()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--body_size', nargs='*',type=float, default=[-1]*3, help="set robot body size")
    parser.add_argument('--inp_points', type=int, default=10, help="set robot body size")
    ...
    parser.add_argument('--a_star_edit', action='store_true', help="set A* edit algorithm")
    parser.add_argument('--a_star_sdf', action='store_true', help="A* in SDF Map")
    parser.add_argument('--random_path', action='store_true', help="set start/end points select policy")

    parser.add_argument('--a_star_sampling', action='store_true')
    parser.add_argument('--interp_sampling', action='store_true')
    parser.add_argument('--hybrid_sampling', action='store_true')
    parser.add_argument('--interp_dense', action='store_true')
    ...
    body_lims = np.array([
        [-opt.body_size[0], opt.body_size[0] ],
        [-opt.body_size[1], opt.body_size[1] ],
        [-opt.body_size[2], opt.body_size[2] ]
    ])

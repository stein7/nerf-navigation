...
class Planner:
    def __init__(self, start_state, end_state, cfg, density_fn, opt):
        ...
        self.hybrid_bp_wayp = list()
        self.hybrid_num_wayp = list()

        self.origin_bp_wayp = list()

        self.hybrid_filter_edge = list()

        self.hybrid_waypoints = list()
        self.origin_waypoints = list()
    ...
    def a_star_init(self, plan_mode=None, model=None, approx=False):
        ...
        kernel_size = 5 # 100/5 = 20. scene size of 2 gives a box size of 2/20 = 0.1 = drone size
        if not approx   : output = self.nerf(coods)
        else            : output = self.sr_approx_nerf(coods, model)

        # Evaluate ich range for offloading
        if self.opt.eval_ich_range:
            output_astar = self.sr_approx_nerf(coods,model)
            #sigma_quant = self.sr_eval_ich_range( coods , model ) 

        # Evaluate Accuracy
        #if self.opt.eval_accuracy and approx:
        #    self.sr_compressor_accuracy( coods, model, output )
        
        # Eval A STAR & BP Accuracy
        if self.opt.eval_compressor:
            output = self.nerf(coods)
            self.sr_compressor_accuracy( coods, model, output )
        ...
        for idx in range(self.opt.total_path_num):
            ...
            if plan_mode == "hybrid":
                self.hybrid_waypoints.append( len(path) )
            else:
                self.origin_waypoints.append( len(path) )
    ...
    def sr_save_tunning_poses( self, filename, plan_mode=None):
        self.save_poses(self.basefolder /  "replan_poses" / filename, plan_mode)
        print(f'Save poses to {self.basefolder}/replan_poses/{filename}')
...
######################################## SR NUM GRAD ESTIMATOR #################################################
    def sr_num_grad_selector(self, plan_mode=None, model=None):

        # get collision way points
        outputs0 = self.sr_get_collision_waypoints(model=model)
        ...
        # Get Sigma Distribution --> Object Attention
        # self.sr_obj_attention( outputs0['collision_way_idx'], outputs3['filter'], outputs3['origin_filter'], outputs2['projected_collision_bmap'] )
        # pdb.set_trace()


        # Get Grad Similarity --> Map Attention
        self.sr_map_attention(outputs0['collision_way_idx'])
        pdb.set_trace()

        # Get Filter Edge IDX Info
        self.sr_save_filter_edge_info(plan_mode, outputs4['edge_idx'])
    ...
    def sr_get_collision_waypoints(self, model):
    ...
    def sr_get_edge(self, collision_bmap):
        ...
        origin_filter_result = torch.Tensor()
        ...
        for wpoints in range (collision_bmap.shape[0]):                             # Collision Waypoints Number
            ...
            print(f'img_sobel_y:')
            print(f'{torch.tensor(img_sobel_x>0)}')
            print(f'img_sobel_x:')
            print(f'{torch.tensor(img_sobel_y>0)}')

            sobel_filter = (sobel_filter > 0)
            sobel_filter = torch.tensor( sobel_filter & bmap[wpoints], device="cuda" ).bool()
            if origin_filter_result.shape[0] == 0: origin_filter_result = sobel_filter.clone()
            else: origin_filter_result = torch.cat( [origin_filter_result, sobel_filter], dim=0 )
        ...
        outputs = {
            'filter': filter_result,        # After Preprocessing
            'origin_filter': origin_filter_result
        }
    ...
    def grad_bp(self, x, approx=False, model=None):
        x = x.clone().detach()
        ...
        ### Approx bp mode
            ...
            sigma = model.sr_msb_switch_debug(x).reshape(x.shape[:-1])**2
            sigma.requires_grad_(True)
            sigma.reshape(x.shape[0]).backward( torch.ones((x.shape[0]), device="cuda"))
            autograd_gradient = None
...
######################################## SR GET WAYP TUNNING VEC #################################################
    def sr_grad_selector(self, num_grad, edge_bpgrad, occupancy_bmap, projected_bmap, edge_cos_sim, plan_mode, edge_skip, approx=False, model=None):
        ...
        bpgrad = self.sr_get_body_grad(approx=approx, model=model)

        # Get Wayp Info
        self.sr_save_wayp_info( plan_mode=plan_mode, numgrad1_bpgrad0=numgrad1_bpgrad0 )
    ...
    def sr_get_body_grad(self, approx=False, model=None):
        ...
        autograd, bpgrad, sigma = self.grad_bp( coords, approx, model)
...
######################################## SR WAYP TUNNING #################################################
    def sr_wayp_tunner(self, tunning_vec, plan_mode, approx=False):
        self.states.requires_grad_(False)
        if plan_mode == "hybrid"            : self.states[:,:3] = self.states[:,:3] + self.opt.tunning_vec_dis * tunning_vec
        elif plan_mode == "bp" and approx   : self.states[:,:3] = self.states[:,:3] + self.opt.bp_tunning_vec_dis * tunning_vec
...
################################################################################################################
    ...
    def sr_save_wayp_info(self, plan_mode, numgrad1_bpgrad0):
        if plan_mode == "hybrid":
            self.hybrid_num_wayp.append( numgrad1_bpgrad0.sum() )
            bpgrad1_numgrad0 = torch.logical_not( numgrad1_bpgrad0 ) 
            self.hybrid_bp_wayp.append( bpgrad1_numgrad0.sum() )
        else:
            self.origin_bp_wayp.append( torch.logical_not(numgrad1_bpgrad0) )

    def sr_save_filter_edge_info(self, plan_mode, edge_idx):
        if plan_mode == "hybrid":
            self.hybrid_filter_edge.append( edge_idx )

    def sr_get_wayp_info(self):
        return self.hybrid_num_wayp, self.hybrid_bp_wayp

    def sr_get_filter_edge_info(self):
        return self.hybrid_filter_edge

    def sr_get_total_wayp_num(self):
        return self.hybrid_waypoints, self.origin_waypoints

    def sr_eval_ich_range(self, coord, model):
        #out = model.sr_ich_debug( coord )
        coord = coord.clone().detach().reshape(-1,3)
        coord.requires_grad_(True)
        sigma = model.sr_ich_debug( coord )
        sigma.requires_grad_(True)
        sigma.backward( torch.ones((coord.shape[0]), device="cuda"), retain_graph=True )
        return sigma

    
    def sr_compressor_accuracy(self, coord, model, accurate_output):
        # ====================== Occupancy Accuracy ============================== #
        linspace = torch.linspace(-1,1, 100) #PARAM extends of the thing
        coods = torch.stack( torch.meshgrid( linspace, linspace, linspace ), dim=-1)
        rot = torch.tensor([[0., 0., 1.], [1., 0., 0.], [0., 1., 0.]], device=device, dtype=torch.float32)
        coords = coods.reshape(-1,3) @ rot

        a_star_sigma = model.sr_a_star(coords).reshape(coods.shape[:-1])
        approx_sigma = model.sr_approx_debug(coords).reshape(coods.shape[:-1])

        a_star_occupied = a_star_sigma > 0.3
        golden_a_star_occupied = accurate_output > 0.3
        approx_occupied = approx_sigma > 0.3
        print(f'Occupied Accuracy [A* Mode]: {(a_star_occupied==golden_a_star_occupied).sum()/a_star_sigma.shape[0]/a_star_sigma.shape[1]/a_star_sigma.shape[2]}')
        print(f'Occupied Accuracy [Approx ]: {(approx_occupied==golden_a_star_occupied).sum()/a_star_sigma.shape[0]/a_star_sigma.shape[1]/a_star_sigma.shape[2]}')
        del golden_a_star_occupied
        del coords
        del linspace
        del approx_occupied
        pdb.set_trace()
        ...
        # ======================= Gradient Accuracy ============================ #
        linspace = torch.linspace(-1,1, 40) #PARAM extends of the thing
        coods = torch.stack( torch.meshgrid( linspace, linspace, linspace ), dim=-1)
        rot = torch.tensor([[0., 0., 1.], [1., 0., 0.], [0., 1., 0.]], device=device, dtype=torch.float32)
        coords = coods.reshape(-1,3) @ rot
        ...
        ### Get Accurate BP
        accurate_sigma = model.sr_accurate_debug(coords).reshape(coods.shape[:-1])
        accurate_sigma.requires_grad_(True)
        accurate_sigma.reshape(coords.shape[0]).backward( torch.ones((coords.shape[0]), device="cuda"))
        np.savetxt('./result/accurate_grad.csv', coords.grad.detach().cpu().numpy(), delimiter=",")
        coords.grad.zero_()
        del accurate_sigma

        ### Get Approx BP
        msb_w_switch_sigma = model.sr_msb_switch_debug(coords).reshape(coods.shape[:-1])
        msb_w_switch_sigma.requires_grad_(True)
        msb_w_switch_sigma.reshape(coords.shape[0]).backward( torch.ones((coords.shape[0]), device="cuda"))
        np.savetxt('./result/approx_grad.csv', coords.grad.detach().cpu().numpy(), delimiter=",")
        coords.grad.zero_()

        errAccurate = np.loadtxt( f'./result/accurate_grad.csv', delimiter="," )
        errApprox = np.loadtxt( f'./result/approx_grad.csv', delimiter="," )

        accurate = torch.tensor( errAccurate )
        approx = torch.tensor( errApprox )

        accurate = accurate/torch.norm(accurate,dim=1).reshape(-1,1)
        approx = approx/torch.norm(approx,dim=1).reshape(-1,1)
        
        inner = torch.sum(accurate * approx, 1)
        acos = torch.acos( inner )
        deg = torch.nan_to_num(torch.rad2deg( acos ), nan=0)
        deg_180 = 180-deg
        DEG = deg.clone()
        DEG[deg<deg_180] = deg[deg<deg_180]
        DEG[deg>deg_180] = deg_180[deg>deg_180]
        print(f'avg degree: {DEG.mean()}')
        print(f'occupied points avg degree: {DEG[ msb_w_switch_sigma.reshape(coords.shape[0]) > 0.3 ].mean()}')
        pdb.set_trace()

        del msb_w_switch_sigma
        del accurate
        del approx
        del inner
        del acos
    ...
    def sr_err_comparison(self, fileA_name, fileB_name, model):
        for l in range (model.num_layers):
            pdb.set_trace()
            nameA = fileA_name+str(l)+'.csv' 
            nameB = fileB_name+str(l)+'.csv' 

            errA = np.loadtxt( nameA, delimiter="," )
            errB = np.loadtxt( nameB, delimiter="," )
            pdb.set_trace()

    def sr_approx_nerf(self, coods, model):
        rot = torch.tensor([[0., 0., 1.], [1., 0., 0.], [0., 1., 0.]], device=device, dtype=torch.float32)
        coords = coods.reshape(-1,3) @ rot

        a_star_sigma = model.sr_layer_wise_mode_v0_debug(coords).reshape(coods.shape[:-1])
        a_star_occupied = a_star_sigma > 0.3

        return a_star_occupied

    def sr_obj_attention(self, colli_idx, filter_list, origin_filter, colli_bmap):
        coords = self.sr_body_to_world( self.robot_body, colli_idx )
        sigma = self.nerf( coords )
        sigma = sigma.reshape( colli_idx.shape[0], self.body_nbins[0], self.body_nbins[1], self.body_nbins[2] )
        sigma = sigma.swapdims( 1, 3 )            # collision waypoints num, zaxis_nbins, xaxis_nbins, yaxis_nbins

        pdb.set_trace()

    def sr_map_attention(self, collision_idx):
        # Get bodypoints on waypoints
        body_coods_on_way = self.dh_body_to_world(self.robot_body)          # waypoints, bodypoints, 3(xyz)
        ...
        print(f'collision waypoints index: {collision_idx}') 

        for col_idx in range( body_coods_on_way.shape[0] ):
            body = body_coods_on_way[col_idx]
            autograd, bpgrad, sigma = self.grad_bp( body )
            print(f'collision wayp id{col_idx} sigma: {sigma.mean()}')

            norm_bpgrad = bpgrad / torch.norm(bpgrad, dim=1).reshape(-1,1)
            norm_bpgrad = torch.nan_to_num( norm_bpgrad, nan=0 )
            inner = norm_bpgrad[:,0]        # inner product with [1,0,0]

            acos = torch.acos(inner)
            acos = acos[ torch.logical_not(torch.isnan(acos)) ]

            deg = torch.rad2deg(acos)
            ...
            print(f'collision wayp id{col_idx} variance: {var}')

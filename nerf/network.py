...
class NeRFNetwork(NeRFRenderer):
    ...
    def sr_ich_debug(self, x):
        ...
        for l in range(self.num_layers):
            ...
            self.sigma_net[l].layer = l
            self.sigma_net[l].FF_quant=self.opt.ff_quant
    ...
    def sr_layer_wise_mode_v0_debug(self, x):           # A STAR
        # x: [N, 3], in [-bound, bound]
        x = self.encoder(x, bound=self.bound)
        h = x
        for l in range(self.num_layers):
            ...
            self.sigma_net[l].tuning = False

            self.sigma_net[l].FF_quant = True
            self.sigma_net[l].BP_quant = True
    ...
    def sr_a_star(self, x):           # A STAR
        ...
        for l in range(self.num_layers):
            ...
            self.sigma_net[l].inlier = self.opt.inlier
            self.sigma_net[l].bit_sparsity = self.opt.bit_sparsity

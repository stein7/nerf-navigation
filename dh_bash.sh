...
set_model_path() {
    ...
    declare -gA models
    models["stonehenge"]="$USER_DATA/stone_nerf"
...
single_cmd () { #CMD :: bash dh_bash.sh 0 stonehenge stonehenge
...
parallel_cmd () { #CMD :: bash dh_bash.sh 0 stonehenge stonehenge
    # Iterate over each GPU and dataset
    gpu_id="$1"
    data="$2"
    model="$3"
...
# Practice
CUDA_VISIBLE_DEVICES=0 python simulate.py ${datasets["stonehenge"]} --workspace ${models["stonehenge"]} \
                                     --interp_dense --start_pos 0.2 0.5 0.2 --end_pos -0.6 -0.7 0.2 --body_size 0.06 0.06 0.05 \
                                     --inp_points 15
#hybrid_sampling interp_dense
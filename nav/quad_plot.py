...
class Planner:
    ...
    def a_star_init(self):
        ...
        pdb.set_trace()
        print("#####-------DH--------######")
        # occupied[:, :, :9] = False #100
        # occupied[:, :, 15:] = False #100
        # occupied[:, :, :17] = False #200
        # occupied[:, :, 29:] = False #200
        occupied_Maxpool_array = occupied.cpu().numpy()
        x, y, z = np.where(occupied_Maxpool_array)

        fig, ax1 = plt.subplots(1, 1, subplot_kw={"projection":"3d"})
        ax1.scatter(x, y, z, c='b', marker='o', label='True', alpha=0.5) #100 : 0.5, 200 : 
        ax1.set_xlabel('X')
        ax1.set_ylabel('Y')
        ...
        # output[:, :, 75:] = False #100
        # output[:, :, :90] = False #200
        # output[:, :, 150:] = False #200
        occupied_Origin = output > 0.3
        occupied_Origin_array = occupied_Origin.cpu().numpy()
        x, y, z = np.where(occupied_Origin_array)

        fig, ax1 = plt.subplots(1, 1, subplot_kw={"projection":"3d"})
        ax1.scatter(x, y, z, c='b', marker='o', label='True', alpha=0.01) #100 : 0.01, 200 : 0.006
        ax1.set_xlabel('X')
        ax1.set_ylabel('Y')
        plt.show()
    ...
    def dh_init(self, start, end, points):
        ...
        # 20, 20, 20
        threshold_maxpool = 0.3
        occupied = maxpool(output[None,None,...])[0,0,...] > threshold_maxpool
        grid_size = side//kernel_size


        pdb.set_trace()
        print("#####------- Nerf map size Measurement --------######")
        occupied_Maxpool_array = occupied.cpu().numpy()
        x, y, z = np.where(occupied_Maxpool_array)

        fig, (ax1, ax2, ax3) = plt.subplots(1, 3, subplot_kw={"projection":"3d"}, figsize=(18, 4.8))
        ax1.scatter(x, y, z, c='b', marker='o', label='True', alpha=0.5) #100 : 0.5 & s=36 , 200 : X 
        ax1.set_xlabel('X')
        ax1.set_ylabel('Y')
        ax1.view_init(elev=30, azim=-60)
        ax2.scatter(x, y, z, c='b', marker='o', label='True', alpha=0.5) #100 : 0.5 & s=36 , 200 : X 
        ax2.set_xlabel('X')
        ax2.set_ylabel('Y')
        ax2.view_init(elev=70, azim=-60)
        ax3.scatter(x, y, z, c='b', marker='o', label='True', alpha=0.5) #100 : 0.5 & s=36 , 200 : X 
        ax3.set_xlabel('X')
        ax3.set_ylabel('Y')
        ax3.view_init(elev=70, azim=130)
        # plt.savefig(f'data/maxpool.png')

        threshold_grid = 0.3
        occupied_Origin = output > threshold_grid
        occupied_Origin_array = occupied_Origin.cpu().numpy()
        x, y, z = np.where(occupied_Origin_array)

        fig, (ax1, ax2, ax3) = plt.subplots(1, 3, subplot_kw={"projection":"3d"}, figsize=(18, 4.8))
        ax1.scatter(x, y, z, c='b', marker='o', label='True', alpha=0.002, s=1) #100 : 0.01 & s=36, 200 : 0.004 & s=5(th10) ->1(th0.3) (stone s=10)
        ax1.set_xlabel('X')
        ax1.set_ylabel('Y')
        ax1.view_init(elev=30, azim=-60)
        ax2.scatter(x, y, z, c='b', marker='o', label='True', alpha=0.002, s=1) #100 : 0.01 & s=36, 200 : 0.004 & s=5(th10) ->1(th0.3) (stone s=10)
        ax2.set_xlabel('X')
        ax2.set_ylabel('Y')
        ax2.view_init(elev=70, azim=-60)
        ax3.scatter(x, y, z, c='b', marker='o', label='True', alpha=0.002, s=1) #100 : 0.01 & s=36, 200 : 0.004 & s=5(th10) ->1(th0.3) (stone s=10)
        ax3.set_xlabel('X')
        ax3.set_ylabel('Y')
        ax3.view_init(elev=70, azim=130)
        # plt.savefig(f'data/grid.png')
        # plt.show()
        ...
        def run_length_encoding(array):
            ...
            for i in range(1, len(array)):
                ...
                if array[i] == array[i-1]:
                    ...
                    # If the current element is different from the previous element, add the count and element to the encoded string
                    encoded_string += str(count) + str(array[i-1])
                    encoded_string_vis += str(count) + str(array[i-1]).replace('0', 'L').replace('1', 'H') # nH nL type
                    # encoded_string += '(' + str(count) + ', ' + str(array[i-1]) + ')' # (n,d) type
                    count = 1
                # print(f'i={i}, string={array[i]}, count={count}, enc={encoded_string}')

            # Add the count and element of the last group to the encoded string
            encoded_string += str(count) + str(array[-1])
            encoded_string_vis += str(count) + str(array[-1]).replace('0', 'L').replace('1', 'H') # nH nL type
            # encoded_string += '(' + str(count) + ',' + str(array[-1]) + ')' # (n,d) type

            return encoded_string, encoded_string_vis

        RLE_ex = run_length_encoding( occupied_Origin_array[26, :, 100].astype(int) )[0]
        RLE_ex_vis = run_length_encoding( occupied_Origin_array[26, :, 100].astype(int) )[1]
        RLE_1D = run_length_encoding( occupied_Origin_array.reshape(-1).astype(int)  )[0]
        RLE_1D_vis = run_length_encoding( occupied_Origin_array.reshape(-1).astype(int)  )[1]

        import sys
        sys.set_int_max_str_digits(400000)

        print(f'Threshold : {threshold}')
        print(f'(RLE Length) {len(RLE_1D)}')
        print(f'(RLE bits) { math.ceil(math.log2( int(RLE_1D) )) } b')
        """
        def run_length_encoding(array):
            """
            Convert a sequence of 0s and 1s to its run-length encoded format.
        ...
        count, data = run_length_encoding( occupied_Origin_array.reshape(-1).astype(int).tolist() )
        count_arr = np.array(count)
        data_arr = np.array(data)

        print(f'Threshold : {threshold_grid}')
        ...
        # z-ax 3D -> 2D grid
        occupied_Origin_array_2D = occupied_Origin_array.reshape(-1, occupied_Origin_array.shape[-1] )
